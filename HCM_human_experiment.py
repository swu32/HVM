...
dfm['trialcollect'] = []
dfm[
    'recall_likelihood'] = []  # the likelihood of subjects recalling the exact piece of sequence, given the representation learned so far
...
for sub in np.unique(list(df['ID'])):
    ...
    for trial in range(1, len_train + 3 * len_test + 1):
        ...
        cg, chunkrecord = hcm_learning(proj_seq, cg, learn = True, chunk = True, abstraction = False)  # with the rational chunk models, rational_chunk_all_info(seq, cg)
        ...
        cg, chunkrecord = hcm_learning(proj_recalled_seq, cg, learn=False,
                                       chunk=False, abstraction = False)  # with the rational chunk models, rational_chunk_all_info(seq, cg)
...
csv_save_directory = '/Users/swu/Desktop/research/motif_learning/data/simonsays_ex2/simulation_data_model_transition_recall_leison_variable_motif_theta_0_dot_9.csv'
dfm.to_csv(csv_save_directory, index=False, header=True)

dfs_csv_save_directory = '/Users/swu/Desktop/research/motif_learning/data/simonsays_ex2/simulation_data_model_transition_recall_leison_variable_motif_individualkey_theta_0_dot_9.csv'
